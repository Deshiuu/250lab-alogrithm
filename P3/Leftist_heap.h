...
int Leftist_heap<Type>::null_path_length() const{
	int z = root_node->null_path_length();
	return z;
}
...
void Leftist_heap<Type>:: push( Type const &obj ){
	...
	Leftist_node<Type> *insert_node = new Leftist_node<Type>(obj);
	heap_size++;
	root_node -> push(insert_node, root_node);
...
Type Leftist_heap<Type>:: pop(){
	heap_size--;
	//Leftist_node<Type> *L_node = root_node.*left();
	//Leftist_node<Type> *R_node = root_node.*right();
	//int n1 = L_node.retrieve();
	//int n2 = L_node.retrieve();
	if(empty()){
		throw underflow();
	}
	
	
	Type root_value = root_node->retrieve();
	
	Leftist_node<Type>* template_node = root_node;
	
	root_node=root_node->left();
	
	root_node->push(template_node->right(),root_node);
	
	delete template_node;
